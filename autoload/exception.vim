...
function! exception#format_traceback(traceback, ...)
  ...
  else  " exception generated by default scheme
    if replace && extra != ''
      let msg = extra
    else
      let msg = msg == ''? extra : extra . msg . ' '
    endif
...
function! exception#exception(mode, type, msg, sfile, slnum)
  if a:mode == 'new' || a:mode == '!' || v:exception == ''
    ...
    let tbinfo = exception#traceback(a:msg, a:mode == 'replace')
    let s:last = tbinfo
  endif
  return s:last
endfunction
